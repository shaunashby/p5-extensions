...
namespace Pix {
  ...
  try  
    ...
    std::cerr << "[Image]: Error in construction - member initialization failed." << std::endl;
  }
  
  Image::Image(const char * path) /* Default image sizes */
  try  
    : m_wcs(0), m_sizex(20), m_sizey(20) {
      _init(path);
    } catch ( ... ) {
  ...
  const char * Image::instrument() const {
    return m_instrument;
  }
  ...
  WorldCoor* Image::wcs() const {
    return m_wcs;
  }
  
  void Image::_init(const char * path) {
